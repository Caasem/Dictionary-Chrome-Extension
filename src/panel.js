...
chrome.storage.local.get(['lastFront', 'lastBack'], function(result) {
    if (result.lastFront) {
    ...
    checkAnkiConnection();
    loadDecks();
    updateQueueStatus(); // Check queue status on load
});
...
function updateDeckSelect() {
    ...
    availableDecks.forEach(deck => {
        ...
        if (deck === 'Arabic Vocabulary') option.selected = true;
        ...
        if (tabs && tabs[0]) {
            chrome.tabs.sendMessage(tabs[0].id, {
                ...
                if (response && response.success) {
                    ...
                    updateQueueStatus(); // Update queue count
                    setTimeout(() => window.close(), 1500);
                } else {
